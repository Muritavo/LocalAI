...
def load_unet(key: str, float16: bool = False):
    """Load the stable diffusion UNet from Hugging Face Hub."""

    # Download the config and create the model
    unet_config = _MODELS_FILES["unet_config"]
    ...
    unet_weights = _MODELS_FILES["unet"]
...
def load_text_encoder(key: str, float16: bool = False):
    """Load the stable diffusion text encoder from Hugging Face Hub."""

    # Download the config and create the model
    text_encoder_config = _MODELS_FILES["text_encoder_config"]
    ...
    # Download the weights and map them into the model
    text_encoder_weights = _MODELS_FILES["text_encoder"]
...
def load_autoencoder(key: str, float16: bool = False):
    """Load the stable diffusion autoencoder from Hugging Face Hub."""

    # Download the config and create the model
    vae_config = _MODELS_FILES["vae_config"]
...
def load_diffusion_config(key: str):
    """Load the stable diffusion config from Hugging Face Hub."""

    diffusion_config = _MODELS_FILES["diffusion_config"]
...
def load_tokenizer(key: str):
    ...
    vocab_file = hf_hub_download(key, _MODELS_FILES["tokenizer_vocab"])
    with open(vocab_file, encoding="utf-8") as f:
        vocab = json.load(f)

    merges_file = hf_hub_download(key, _MODELS_FILES["tokenizer_merges"])
